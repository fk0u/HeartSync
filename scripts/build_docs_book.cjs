...
// Read all chapter files 01..50 + DIAGRAMS.md
...
<head>
  ...
  <style>
    @media print {
      ...
      .card-box { background: white !important; border: 1px solid #cbd5e1 !important; color: black !important; }
    }
  </style>
</head>
<body class="bg-slate-900 text-slate-100 min-h-screen font-sans flex">
  ...
  <aside class="w-80 bg-slate-950 border-r border-slate-800 h-screen sticky top-0 overflow-y-auto p-4 no-print shrink-0">
    <div class="flex items-center gap-3 mb-6 px-2">
      ...
      <div>
        ...
        <p class="text-xs text-slate-400">Enterprise Documentation (50 Bab)</p>
      </div>
    </div>
  ...
  <main class="flex-1 p-8 max-w-5xl mx-auto space-y-12">
    ${chaptersData.map((c, i) => {
      return `
        <article id="chapter-${i+1}" class="card-box bg-slate-800/60 backdrop-blur-md rounded-3xl p-8 border border-slate-700/60 shadow-xl page-break">
...
// Create Master Typst Source with clean syntax replacements
  ...
  return md
    ...
    .replace(/\$(.*?)\$/g, (m, p1) => {
      let math = p1
        ...
        .replace(/\\frac\{(.*?)\}\{(.*?)\}/g, '($1 / $2)')
        .replace(/\\text\{(.*?)\}/g, '"$1"')
        .replace(/(?<!["\w\\])(MAP|PP|SD|CV|Systolic|Diastolic)(?!["\w])/g, '"$1"')
        .replace(/\\ge/g, '>=')
        .replace(/\\le/g, '<=');
      return `$ ${math} $`;
    })
    .replace(/\*\*(.*?)\*\*/g, '*$1*') // Convert Markdown bold to Typst bold
    .replace(/```mermaid[\s\S]*?```/g, '[Diagram Mermaid - Lihat Web Portal]') // Replace mermaid block in typst
    .replace(/\[(.*?)\]\((.*?)\)/g, '#link("$2")[$1]'); // Convert markdown links
}

chaptersData.forEach((c) => {
  const converted = markdownToTypst(c.content);
  typstContent += `\n#pagebreak()\n${converted}\n`;
});
...
try {
  ...
  console.error("Typst compilation status:", err.message);
}
